...
class mTrust():
    ...
    def __init__(self):
        ...
        for i in range(1000):
            for j in range(1000):
                self.R_train_ui[i,j]=0
        for i in range(len(self.R_train[:,0])):
            for j in range(len(self.R_train[:,1])):
                #print i,j
                self.R_train_ui[(self.R_train[:,0][i],self.R_train[:,1][j])]=self.R_train[:,3][i]
        ...
        for i in range(1000):
            for j in range(1000):
                self.R_test_ui[(i,j)]=0
        for i in range(len(self.R_test[:,0])):
            for j in range(len(self.R_test[:,1])):
                #print i,j
    ...
    def Nui(self):
        ...
        #with open('../data/ciao/ciaonui') as f:
            ...
            for line in content:
                ...
                value = [r for r in value if r < 1000]
                #print key[0],key[1]
                #print key
                #print value
                #print "\n"
                if key[0] < 1000 and key[1] < 1000:
                    self.V[key] = value
